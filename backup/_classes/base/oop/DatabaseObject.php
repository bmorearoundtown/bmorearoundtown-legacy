...
//				throw new ChurchIdDoesNotMatchException($this);
		}
	...
	public function insertIntoDatabase($autonumber = true, $debug = false, $skipOwnership = false) {
		...
		if (!$skipOwnership && !$this->checkOwnership()) throw new ChurchIdDoesNotMatchException($this);
	 ...
	 *	@return	bool	The results of the query (true if successful, false otherwise)
	 */
	public function removeFromDatabase($ignoreBlankChurchId = false) {
	
		if (!$this->checkOwnership($ignoreBlankChurchId)) return false;
	...
	/**
	 *	Load all records in the current table, filtering only by Church ID.
	 *
	 *	@param	int		$churchId		The Church ID to filter by
	 *	@param	bool	$autoload		Whether to automatically load the first record (used when only expecting one record)
	 *	@param	bool	$debug 			When set to true, the query string will be echoed and script execution will halt
	 *	@param	mixed	$sort			Optional sort string or array of sort strings (@fieldName ASC|DESC)
	 *	@return	DatabaseObject		If $autoload is set to true, a {@link DatabaseObject} will be returned
	 */
	public function loadByChurchId($churchId, $autoload = false, $debug = false, $order = null) {
	
		$queryOrder = "";
	
		if($order != null) {
			if(is_array($order)) {
				$queryOrder = " ORDER BY " . implode(",", $order) . " ";
		...
		$this->query("SELECT	*
					  ...
					  $queryOrder ", $debug);
		if ($autoload) return $this->loadNext();
	}
	
	/**
	 *	Load all records in the current table, filtering only by Family ID.
	 *
	 *	@param	int		$familyId		The Family ID to filter by
	 *	@param	bool	$autoload		Whether to automatically load the first record (used when only expecting one record)
	 *	@param	bool	$debug 			When set to true, the query string will be echoed and script execution will halt
	 *	@return	DatabaseObject		If $autoload is set to true, a {@link DatabaseObject} will be returned
	 */
	public function loadByFamilyId($familyId, $autoload = false, $debug = false) {
		$this->query("SELECT	*
					  FROM		$this->tableName
					  WHERE		@familyId = '$familyId'", $debug);
		if ($autoload) return $this->loadNext();
	}
	
	/**
	 *	Load all records in the current table, filtering only by Member ID.
	 *
	 *	@param	int		$memberId		The Member ID to filter by
	 *	@param	bool	$autoload		Whether to automatically load the first record (used when only expecting one record)
	 *	@param	bool	$debug 			When set to true, the query string will be echoed and script execution will halt
	 *	@return	DatabaseObject		If $autoload is set to true, a {@link DatabaseObject} will be returned
	 */
	public function loadByMemberId($memberId, $autoload = false, $debug = false) {
		$this->query("SELECT	*
					  FROM		$this->tableName
					  WHERE		@memberId = '$memberId'", $debug);
		if ($autoload) return $this->loadNext();
	}
	
	/**
	 *	Delete all records in the current table by Member ID.
	 *
	 *	@param	int		$memberId				The Member ID to filter by
	 *	@param	bool	$ignoreBlankChurchId	When set to true, a blank Church ID won't throw an exception (used for legacy data)
	 *	@param	bool	$debug 					When set to true, the query string will be echoed and script execution will halt
	 *	@return	bool	True if the deletion was successful, false on error
	 */
	public function deleteByMemberId($memberId, $ignoreBlankChurchId = false, $debug = false) {

		$this->loadByMemberId($memberId, false, $debug);
		
		while ($this->loadNext())
			if (!$this->delete($ignoreBlankChurchId))
				return false;
	...
	/**
	 *	Delete all records in the current table by Family ID.
	 *
	 *	@param	int		$familyId				The Family ID to filter by
	 *	@param	bool	$ignoreBlankChurchId	When set to true, a blank Church ID won't throw an exception (used for legacy data)
	 *	@param	bool	$debug 					When set to true, the query string will be echoed and script execution will halt
	 *	@return	bool	True if the deletion was successful, false on error
	 */
	public function deleteByFamilyId($familyId, $ignoreBlankChurchId = false, $debug = false) {

		$this->loadByFamilyId($familyId, false, $debug);
	...
	public function checkOwnership($ignoreBlankChurchId = false) {
	
		if ($GLOBALS['ignoreOwnership']) return true;
		
		if ($this->_fields['churchId'] === 0 && !$ignoreBlankChurchId)
			throw new NoChurchIdException($this);

		return !isset($this->_fields['churchId']) || (isset($this->_fields['churchId']) && $this->churchId == $GLOBALS['churchId']);
